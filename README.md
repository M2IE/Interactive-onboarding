...
- Имплементация джижка по генерации PDF
- Описание OpenApi спецификации
...
### Frontend-монорепозиторий и Feature-Sliced Design

Frontend организован через npm workspaces: развёртываемое SPA находится в `apps/web`, а SDK, API-клиент, общие контракты и UI-примитивы вынесены в отдельные пакеты. Внутри приложения используется Feature-Sliced Design, а библиотечные пакеты сохраняют независимую от приложения структуру. Это позволяет отдельно тестировать, собирать и развивать административную панель, демонстрационный классифайд и встраиваемый SDK.

### Изоляция виджета от приложения-хоста

SDK не зависит от административной UI-библиотеки и внутренних модулей демонстрационного приложения. Стили виджета имеют собственный префикс, API передаётся через клиент-адаптер, а SPA-навигация подключается через функцию `navigate`. Благодаря этому один и тот же пакет можно использовать в React-приложении или подключить через императивный API, не связывая его с конкретным роутером и дизайн-системой.

### Динамическая конфигурация онбординга

Подсказки не зашиты в компоненты демонстрационного сайта: SDK получает опубликованный сценарий для текущего URL через Widget API. Транспорт скрыт за типизированным API-клиентом, а `real` и `mock` режимы выбираются явно через конфигурацию окружения. Это сохраняет автономную демонстрацию и позволяет подключать тот же runtime к настоящему бэкенду без изменения UI виджета.
...
- **Кнопка «Назад» между страницами** - после перехода по `nextUrl` на новую страницу SDK не может вернуться к подсказке с предыдущей страницы. Переход «Назад» работает только между подсказками в пределах одной страницы.
